...
const HEIGHT_CARD = Dimensions.get('window').height / 3.5
      ...
      <Card
        ...
        <Card f={1} overflow="hidden" br="$2">
          <ZStack f={1}>
            ...
            {item?.rating && (
              <Stack
                h={getTokens().size[1].val}
                position="absolute"
                left={0}
                bottom={0}
                right={0}
                jc="center">
                <Stack h={getTokens().size[1].val} bg="$color1" o={0.7} />
                <XStack position="absolute" right={2.5} ai="center" gap="$2">
                  <Text fontWeight="$6" fontSize="$4" color="$color12" top={2}>
                    {item.rating}
                  </Text>
                  <Star size="$1" color="$yellow10" fill={theme.yellow6.val} />
                </XStack>
              </Stack>
            )}
          </ZStack>
